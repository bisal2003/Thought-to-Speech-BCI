...
# Use the new 3-band wavelet and ResNet18-based CNN
from resnet_cnn import EEGResNet18
from wavelets_res import WaveletTransform3Channel
from data_imagine import get_dataset

class EEGclassification(torch.nn.Module):
    """
    Single-stream: pkl → wavelet (3 bands) → ResNet18 → MLP classifier
    """
    def __init__(self, chans=122, samples=1651, cls=39, dropout=0.25):
        super().__init__()
        self.wavelet_encoder = WaveletTransform3Channel()
        self.resnet = EEGResNet18(chans=chans, samples=samples, out_dim=512)
        self.dropout = torch.nn.Dropout(dropout)
        self.linear = torch.nn.Linear(512, cls)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a single-stream 3-band EEG ResNet18 classification model.")
    ...
    parser.add_argument('--batch', type=int, default=16, help="Batch size for training and validation.")
    parser.add_argument('--checkpoint_path', type=str, default='checkpoints_resnet', help="Directory to save model checkpoints.")
    ...
    parser.add_argument('--dropout', type=float, default=0.25, help="Dropout rate for the classifier.")
    ...
    model = EEGclassification(
        chans=122, samples=args.samples, cls=args.cls, dropout=args.dropout
    ).to(device)
