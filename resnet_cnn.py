...
class EEGcnn(nn.Module):
    ...
    """
    def __init__(self, Chans=122, F1=8, D=2, F2=16, kernLength1=125, kernLength2=25, P1=2, P2=5, dropoutRate=0.5, in_channels=3):
        super().__init__()
...
class EEGResNet18(nn.Module):
    """
    ResNet18 backbone for EEG 3-band (RGB) image input.
    Input: (N, 3, Chans, Samples)
    Output: (N, feature_dim)
    """
    def __init__(self, chans=122, samples=1651, out_dim=512):
        super().__init__()
    ...
    def forward(self, x):
        # x: (N, 3, Chans, Samples)
        # ResNet expects (N, 3, H, W), so Chans=H, Samples=W
